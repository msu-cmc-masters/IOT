#!/usr/bin/env python3
"""
Network Traffic Malware Detection — Three-Stage Analysis
  1. Binary Classification (MLP + Random Forest)
  2. Permutation Importance Model Explainability
  3. FGSM White-Box Adversarial Attack
"""
...
# Configure Chinese-compatible fonts to avoid garbled text in charts
import platform
...
class TorchMLPWrapper(BaseEstimator, ClassifierMixin):
    """
    sklearn-compatible PyTorch MLP wrapper
    Provides fit / predict methods for use with sklearn.inspection.permutation_importance
    """
...
def load_and_merge_data(data_path):
    """Read all CSV files (pipe-delimited) under data/ and merge into a single DataFrame"""
    csv_files = sorted([
        f for f in os.listdir(data_path) if f.endswith(".csv")
    ])
    if not csv_files:
        print("[Error] No CSV files found under data/, attempting to download via kagglehub...")
        import kagglehub
        import shutil
    ...
    print(f"  Merge complete, data shape: {merged.shape}")
    return merged


def explore_data(df):
    """Data exploration: dtypes, missing values, label distribution"""
    # Column dtypes
    ...
    print(f"\n  Original label class distribution ({len(label_counts)} classes):")
...
def binarize_label(df):
    """Binarize labels: Benign -> 0, everything else -> 1"""
...
def preprocess_data(df):
    ...
    print(f"  Converted {[c for c in str_to_num_cols if c in df.columns]} to numeric")
    ...
    # 5. Fill missing values: numerical with median, categorical with mode
    ...
    # 7. IQR outlier clipping (numerical columns only, exclude label_binary)
    ...
    print(f"  IQR outlier ratio: {outlier_ratio:.4f} ({total_outliers}/{total_cells})")

    return df, numeric_cols, categorical_cols


def sample_data(df, n_per_class=100000):
    """Sample: n_per_class Benign and Malicious records each"""
    ...
    print(f"\n  After sampling, data shape: {sampled.shape}")
    print(f"  Sampled class distribution: {sampled['label_binary'].value_counts().to_dict()}")
    return sampled
...
def select_features(df, feature_cols):
    """Feature selection: remove highly correlated features + SelectFromModel"""
    X = df[feature_cols]
    y = df["label_binary"]
    ...
    print(f"\n  Feature pairs with |correlation| > 0.95: {len(high_corr)} pairs")
    for a, b, v in high_corr[:10]:
        print(f"    {a} <-> {b}: {v:.4f}")

    # From each group of highly correlated features, keep the one with higher correlation to target
    drop_from_corr = set()
    ...
    if drop_from_corr:
        print(f"  Removing highly correlated features: {list(drop_from_corr)}")

    X_reduced = X.drop(columns=list(drop_from_corr), errors="ignore")
    feature_cols = list(X_reduced.columns)

    # SelectFromModel (based on Random Forest)
    ...
    print(f"  SelectFromModel retained features: {len(selected_features)} / {len(feature_cols)}")
...
def train_mlp(X_train, y_train, input_dim):
    """MLP hyperparameter search + training"""
    ...
    print(f"\n  MLP best hyperparameters: hidden_layers={hidden}, lr={lr}, val_acc={best_acc:.4f}")
    return best_model
...
def train_random_forest(X_train, y_train):
    ...
    print(f"\n  RF best params: {grid.best_params_}, CV Acc: {grid.best_score_:.4f}")
...
def evaluate_model(model, X_test, y_test, name, plots_dir):
    """Evaluate model, output metrics, save charts"""
    if hasattr(model, "predict"):
        y_pred = model.predict(X_test)
    else:
        y_pred = model(X_test)

    # Handle probability output (if predict_proba is available)
...
def plot_roc_curves(results, y_test, plots_dir):
    ...
    for o in others:
        ...
        print(f"    {best['name']} beats {o['name']} by {diff * 100:.1f} percentage points")
    return best
...
def compare_top_features(mlp_top, rf_top, plots_dir):
    """Side-by-side comparison of MLP and RF Top-N features"""
    ...
    # Sort by sum of both importance scores
    ...
    n_top = len(mlp_top)
    print(f"\n  MLP and RF Top-{n_top} intersection: {len(intersection)}/{n_top}")
    if intersection:
        print(f"    Common features: {intersection}")
    print(f"    MLP unique: {mlp_set - rf_set}")
    print(f"    RF unique: {rf_set - mlp_set}")
    if len(intersection) >= 8:
        print("  Conclusion: Both models rely on highly consistent features.")
    else:
        print("  Conclusion: Feature importance rankings differ significantly between the models, "
              "suggesting different model types utilize features differently.")


# ============================================================================
# Part 3: FGSM Adversarial Attack
# ============================================================================
def fgsm_attack(model_wrapper, X_malicious, y_malicious_true, epsilon):
    """
    Perform FGSM white-box attack on the MLP model
    x_adv = x + epsilon * sign(grad_x L(x, y_target))
    y_target = 0 (trick the model into predicting Benign)
    """
    ...
    # Forward pass to get predictions after attack
    with torch.no_grad():
    ...
    # Successful attack: originally malicious (1), predicted as benign (0)
...
def evaluate_attack_impact(model_wrapper, X_adv, X_benign_test, y_benign_test,
    ...
    """Evaluate overall model metrics after attack"""
    # Combine: benign (original predictions) + adversarial malicious
...
def plot_attack_success_curve(epsilons, success_rates, plots_dir):
    """Attack success rate vs epsilon line chart"""
    ...
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"  Attack success rate chart saved: {path}")
...
def plot_attack_confusion_matrix(y_true, y_pred, plots_dir):
    ...
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"  Post-attack confusion matrix saved: {path}")
...
def plot_summary_table(mlp_results, rf_results, attack_metrics_after,
    ...
    """Generate comprehensive analysis summary table, save as PNG"""
    fig, ax = plt.subplots(figsize=(14, 8))
    ax.axis("off")

    col_labels = ["Metric", "MLP", "Random Forest", "MLP (after attack)"]
    cell_text = []

    # ---- Model Performance Comparison ----
    cell_text.append(["-- Model Performance Comparison --", "", "", ""])
    ...
    cell_text.append([f"-- Adversarial Attack Impact (epsilon={best_eps:.2f}) --", "", "", ""])
    cell_text.append(["Attack Success Rate", f"{success_rate*100:.1f}%", "N/A (tree model)", ""])
    f1_drop = (mlp_results["f1"] - attack_metrics_after["f1"]) / mlp_results["f1"] * 100
    cell_text.append(["F1 Drop", f"{f1_drop:.1f}%", "N/A", ""])

    # ---- Feature Importance Top-10 ----
    cell_text.append(["-- Feature Importance Top-10 --", "", "", ""])
    intersection = set(mlp_top_dict.keys()) & set(rf_top_dict.keys())
    cell_text.append(["MLP & RF Intersection", f"{len(intersection)}/10", "", ""])
    ...
    cell_text.append(["-- Conclusion --", "", "", ""])
    cell_text.append(["Best Classification Model", f"{best_name} (F1={best_f1:.4f})", "", ""])
    cell_text.append(["MLP Robustness", f"FGSM(eps={best_eps:.2f}) F1 drop {f1_drop:.1f}%", "", ""])
    ...
    for (row, col), cell in table.get_celld().items():
        ...
        elif "Conclusion" in str(cell.get_text()):
            cell.set_facecolor("#D9E2F3")
            cell.set_text_props(fontweight="bold")
        elif any(kw in str(cell.get_text()) for kw in ["Performance Comparison", "Attack Impact", "Feature Importance"]):
            cell.set_facecolor("#D9E2F3")
            cell.set_text_props(fontweight="bold")
    ...
    plt.close(fig)
    print(f"\nSummary comparison table saved: {path}")
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description="Network Traffic Malware Detection — Three-Stage Analysis")
    parser.add_argument("--data_path", type=str, default="data", help="Dataset directory path")
    parser.add_argument("--test_size", type=float, default=0.2, help="Test set proportion")
    parser.add_argument("--epsilon", type=float, default=None,
                        help="FGSM perturbation strength, if specified skip epsilon sweep and use this value directly")
    ...
    # Part 1: Classifier
    # ========================================================================
    print_sep("Part 1: Classifier (MLP vs Random Forest)")
    ...
    print(f"  After SMOTE, data shape: {X.shape}, class distribution: {Counter(y)}")
    ...
    print("\n[1.4] Model Training")
    print("  Training MLP (PyTorch + MPS)...")
    ...
    # Part 2: Permutation Importance
    # ========================================================================
    print_sep("Part 2: Permutation Importance Explanation")
    print("\n  Permutation Importance principle: randomly shuffle the values of a feature")
    print("  and observe how much the model's accuracy drops. The more it drops,")
    print("  the more important that feature is. Model-agnostic, works for any model.")
    ...
    print(f"  Training & Test set Top-{len(sorted_train)} overlap: {overlap}/{len(sorted_train)}")
    ...
    print("\n[2.4] MLP vs RF Feature Importance Comparison")
    compare_top_features(mlp_top_dict, rf_top_dict, plots_dir)

    # ========================================================================
    # Part 3: FGSM Adversarial Attack
    # ========================================================================
    print_sep("Part 3: FGSM White-Box Adversarial Attack")
    print("\n  FGSM (Fast Gradient Sign Method): leverages the model's gradient information,")
    print("  adding a tiny perturbation along the gradient direction on the input data,")
    print("  flipping the model's prediction.")
    print("  Core formula: x_adv = x + eps * sign(grad_x L(x, y_target))")
    print("  Only MLP (differentiable) can be attacked; RF (tree model) cannot.")

    # Extract malicious samples from test set
    ...
    print(f"\n  Malicious samples: {len(X_malicious)}, Benign samples: {len(X_benign)}")

    if args.epsilon is not None:
        # Use specified epsilon directly
        print(f"\n[3.3] Using specified epsilon = {args.epsilon}")
        ...
        print(f"  Average L2 perturbation: {l2_dist:.4f}")
        ...
        for eps in epsilons:
            ...
            marker = "  <-- best" if success_rate > 0.5 else ""
            print(f"    epsilon={eps:.2f} -> Attack Success: {success_rate*100:.2f}%{marker}")

        # Find best epsilon (smallest epsilon with success rate > 50%)
        ...
        if valid:
            ...
            print("  Warning: No epsilon achieved attack success rate > 50%, selecting highest success rate")
    ...
    print(f"\n  {'Metric':<12} {'Before':<12} {'After':<12} {'Change':<12}")
    print(f"  {'-' * 48}")
    ...
    print(f"\n  Conclusion: FGSM white-box attack reduced MLP F1 from {f1_before:.4f} to {f1_after:.4f},")
    print(f"  a drop of {f1_drop_pct:.1f}%. Although MLP has gradients available (enabling white-box")
    print(f"  attacks), the model itself lacks resistance to small perturbations.")
