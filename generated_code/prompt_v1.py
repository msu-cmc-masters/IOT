...
class TorchMLPWrapper(BaseEstimator, ClassifierMixin):
    """
    sklearn 兼容的 PyTorch MLP wrapper
    提供 fit / predict 方法，方便接入 sklearn.inspection.permutation_importance
    """
...
def load_and_merge_data(data_path):
    ...
    if not csv_files:
        print("[错误] data/ 目录下没有找到 CSV 文件，尝试通过 kagglehub 下载...")
        import kagglehub
        import shutil
    ...
    print(f"  合并完成，数据形状: {merged.shape}")
    return merged
...
def explore_data(df):
    ...
    print(f"\n  原始 label 类别分布 (共 {len(label_counts)} 种):")
...
def preprocess_data(df):
    ...
    print(f"  已将 {[c for c in str_to_num_cols if c in df.columns]} 转为数值型")
    ...
    print(f"  类别型列 ({len(categorical_cols)}): {categorical_cols}")

    # 5. 填充缺失值：数值列用中位数，类别列用众数
    ...
    # 7. IQR 异常值截断（仅数值列，排除 label_binary）
    ...
    print(f"  IQR 异常值比例: {outlier_ratio:.4f} ({total_outliers}/{total_cells})")

    return df, numeric_cols, categorical_cols


def sample_data(df, n_per_class=100000):
    """采样：Benign 和 Malicious 各 n_per_class 条"""
    ...
    print(f"\n  采样后数据形状: {sampled.shape}")
    print(f"  采样后类别分布: {sampled['label_binary'].value_counts().to_dict()}")
    return sampled
...
def select_features(df, feature_cols):
    ...
    print(f"\n  相关系数 > 0.95 的特征对: {len(high_corr)} 对")
    for a, b, v in high_corr[:10]:
        print(f"    {a} ↔ {b}: {v:.4f}")

    # 从每组高度相关特征中保留与目标相关性更高的
    drop_from_corr = set()
    ...
    print(f"  SelectFromModel 后保留特征: {len(selected_features)} / {len(feature_cols)}")
...
def train_mlp(X_train, y_train, input_dim):
    ...
    print(f"\n  MLP 最优超参数: hidden_layers={hidden}, lr={lr}, val_acc={best_acc:.4f}")
    return best_model
...
def train_random_forest(X_train, y_train):
    ...
    print(f"\n  RF 最优参数: {grid.best_params_}, CV Acc: {grid.best_score_:.4f}")
...
def plot_roc_curves(results, y_test, plots_dir):
    ...
    print(f"\n  最佳模型: {best['name']}（F1 weighted: {best['f1_weighted']:.4f}）")
    for o in others:
        diff = best["f1_weighted"] - o["f1_weighted"]
        print(f"    {best['name']} 领先 {o['name']} {diff * 100:.1f} 个百分点")
    return best


# ============================================================================
# 第二部分：Permutation Importance
# ============================================================================
...
def compare_top_features(mlp_top, rf_top, plots_dir):
    ...
    n_top = len(mlp_top)
    print(f"\n  MLP 和 RF Top-{n_top} 交集: {len(intersection)}/{n_top}")
    if intersection:
        print(f"    共同特征: {intersection}")
    print(f"    MLP 独有: {mlp_set - rf_set}")
    print(f"    RF 独有: {rf_set - mlp_set}")
    if len(intersection) >= 8:
        print("  结论：两个模型依赖的特征高度一致。")
    else:
        print("  结论：两个模型的特征重要性排序存在较大差异，说明不同类型的模型对特征有不同的利用方式。")
...
def fgsm_attack(model_wrapper, X_malicious, y_malicious_true, epsilon):
    """
    对 MLP 模型执行 FGSM 白盒攻击
    x_adv = x + ε · sign(∇_x L(x, y_target))
    y_target = 0（让模型误判为 Benign）
    """
...
def evaluate_attack_impact(model_wrapper, X_adv, X_benign_test, y_benign_test,
    ...
    """评估攻击后整体模型指标"""
    # 合并：benign 保持原始预测 + adversarial malicious
...
def plot_attack_success_curve(epsilons, success_rates, plots_dir):
    ...
    ax.set_xlabel("ε (perturbation strength)")
    ax.set_ylabel("Attack Success Rate")
    ax.set_title("FGSM Attack Success Rate vs ε")
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description="网络流量恶意软件检测三阶段分析")
    parser.add_argument("--data_path", type=str, default="data", help="数据集目录路径")
    parser.add_argument("--test_size", type=float, default=0.2, help="测试集比例")
    parser.add_argument("--epsilon", type=float, default=None,
                        help="FGSM 扰动强度，若指定则跳过 ε 扫描直接用该值")
    ...
    # 第一部分：分类器
    # ========================================================================
    print_sep("第一部分：分类器（MLP vs Random Forest）")
    ...
    print(f"  SMOTE 后数据形状: {X.shape}, 类别分布: {Counter(y)}")
    ...
    # 第二部分：Permutation Importance
    # ========================================================================
    print_sep("第二部分：Permutation Importance 解释")
    print("\n  Permutation Importance 原理：随机打乱某个特征的值，观察模型准确率下降多少。")
    print("  下降越多，该特征越重要。不依赖模型内部结构，适用于任意模型。")
    ...
    print(f"  训练集和测试集 Top-{len(sorted_train)} 交集: {overlap}/{len(sorted_train)}")
    ...
    print("\n[2.4] MLP vs RF 特征重要性对比")
    compare_top_features(mlp_top_dict, rf_top_dict, plots_dir)

    # ========================================================================
    # 第三部分：FGSM 对抗攻击
    # ========================================================================
    print_sep("第三部分：FGSM 白盒对抗攻击")
    print("\n  FGSM（Fast Gradient Sign Method）：利用模型梯度信息，")
    print("  在输入数据上沿梯度方向添加微小扰动，使模型预测结果翻转。")
    print("  核心公式：x_adv = x + ε · sign(∇_x L(x, y_target))")
    print("  只有 MLP（可求梯度）能使用，RF（树模型）无法使用。")
    ...
    print(f"\n  恶意样本数: {len(X_malicious)}, 正常样本数: {len(X_benign)}")
    ...
    if args.epsilon is not None:
        ...
        for eps in epsilons:
            ...
            marker = " ← 最佳" if success_rate > 0.5 else ""
            print(f"    ε={eps:.2f} → Attack Success: {success_rate*100:.2f}%{marker}")
        ...
        if valid:
            ...
            print("  ⚠ 没有 ε 使攻击成功率超过 50%，选择成功率最高的 ε")
